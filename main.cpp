...
bool parse_config(const string &fname){
    ...
    bool single_hero_format = false;
    bool hero_count_explicit = false;
    int max_hero_id_found = 0;
    regex hero_id_regex("^HERO_(\\d+)_HP\\s+.*$");
    
    bool monster_count_explicit = false;
    int max_monster_id_found = 0;
    regex monster_id_regex("^MONSTER_(\\d+)_HP\\s+.*$");
    ...
    for(auto &ln : lines){
        if(ln.rfind("GRID_SIZE",0)==0){
            ...
            hero_count_explicit = true;
        } else if(ln.rfind("MONSTER_COUNT",0)==0){
            istringstream iss(ln); string k; iss>>k>>cfg.monster_count;
            monster_count_explicit = true;
        } else if (ln.rfind("HERO_HP", 0) == 0) {
            single_hero_format = true;
        } else {
            smatch m_hero;
            if(regex_search(ln, m_hero, hero_id_regex)){
                int id = stoi(m_hero[1]);
                if(id > max_hero_id_found) max_hero_id_found = id;
            }
            
            smatch m_monster;
            if(regex_search(ln, m_monster, monster_id_regex)){
                int id = stoi(m_monster[1]);
                if(id > max_monster_id_found) max_monster_id_found = id;
            }
        }
    }
    ...
    if (monster_count_explicit) {
    } else if (max_monster_id_found > 0) {
        cfg.monster_count = max_monster_id_found;
    }
...
int main(int argc, char** argv){
...

    {
        ...
        if(cfg.hero_count > 0 || cfg.monster_count > 0) { // Lógica de espera modificada
